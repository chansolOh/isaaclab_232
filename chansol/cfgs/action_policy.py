...
class ActionPolicy:
    ...
    def make_gripper_bbox(self,idx):
        ...
        if self.type == "parallel_jaw":
            ...
            width_tensor[:,[0,1],1] = -width[:,None,0]
            width_tensor[:,[2,3],1] =  width[:,None,1]
            ...
            close_joint = torch.tensor([self.down_lift_joint,
                                        ...
                                        self.gripper_info["close_rf_joint"], self.gripper_info["close_lf_joint"],0 ],
                                    dtype=torch.float, device=self.device)

            width = close_joint[[1,2]]/180*torch.pi - joints[...,[1,2]]
    ...
    def check_object_lift_up(self, idx):
        ...
        success_idx = (objects_pos - objects_pos_org)[:,2] >0.02
        if len(idx[success_idx])>0:
            bbox = self.make_gripper_bbox(idx[success_idx]).transpose(0,1)
            ...
            bbox_2d             = torch.stack([self.bbox_3d_to_2d(aligned_bbx_3d).cpu() for aligned_bbx_3d in aligned_bbox_3d] )
            ...
            for i,index in enumerate(torch.where(success_idx==success_idx)[0][success_idx].cpu()) :
                bbox_yaw = torch.atan2(bbox_2d[i][0][2][1] - bbox_2d[i][0][1][1], bbox_2d[i][0][2][0] - bbox_2d[i][0][1][0])
                self.output_list.append({
                    "bbox_3d_point" : restored_bbox_3d[i].cpu().tolist(),
                    "bbox_3d_aligned" : aligned_bbox_3d[i].cpu().tolist(),
                    "bbox_2d" :{
                        "bbox":bbox_2d[i].tolist(),
                        "center":bbox_2d[i].mean(dim=1).tolist(),
                        "width":torch.sqrt(torch.sum((bbox_2d[i][0][1] - bbox_2d[i][0][2])**2) ).tolist(),
                        "height":torch.sqrt(torch.sum((bbox_2d[i][0][0] - bbox_2d[i][0][1])**2) ).tolist(),
                        "angle":bbox_yaw.tolist(),
                    },
                    "target_points" : self.target_pos[idx[success_idx][i]].cpu().tolist(),
                    "target_orientation" : quat_to_euler_angles(self.target_rot[idx[success_idx][i]].cpu(), degrees=True).tolist(),
